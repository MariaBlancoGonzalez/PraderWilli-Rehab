...
class DiagonalsScene(Scene):
    def __init__(self, game):
        ...
        self.feet_right = None if game.static_points == None else (
            self.game.static_points.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE].x, self.game.static_points.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ANKLE].y)
        self.feet_left = None if game.static_points == None else (
            self.game.static_points.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE].x, self.game.static_points.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ANKLE].y)
    ...
    def check_visibility(self):
        try:
            visibility = [
                i.visibility for i in self.current_results.pose_landmarks.landmark]
    ...
    def draw(self):
        ...
        if self.calibration:
            pygame.draw.rect(self.game.display,
                             settings.GRANATE, self.box_feet,  5, 0)
    ...
    def create_box_feet(self):
        ...
        rect = pygame.Rect(point_left[0]-settings.FEET_MARGIN, point_left[1] -
                           settings.FEET_MARGIN, width + settings.FEET_BOX, settings.FEET_BOX)
        return rect
    ...
    def body_controller(self, results):
        ...
        if not self.calibration:
            ...
            if self.calibration:
                ...
                self.shoulder_left, self.shoulder_right = get_shoulder_pos(
                    results)
                self.bound_left_hand = (left_hand_bound[0], left_hand_bound[1])
                self.bound_right_hand = (
                    right_hand_bound[0], right_hand_bound[1])
                self.music.play()
                # self.music_playing = True
        # Pantalla de 3,2,1...
        if self.time_instr < 3 and self.calibration and not self.end:
            ...
            bola_permitida_drch = True if (pygame.time.get_ticks(
            )-self.time_right)/1000 >= settings.VELOCIDAD_ENTRE_BOLAS else False
            bola_permitida_izq = True if (pygame.time.get_ticks(
            )-self.time_left)/1000 >= settings.VELOCIDAD_ENTRE_BOLAS else False
            # Crear acierto y fallo
            if len(self.points_left) == 0 and not left_tramp and bola_permitida_izq:

                left_x = random.uniform(
                    self.bound_left_hand[0], self.shoulder_right[0])
                left_y = random.uniform(
                    self.shoulder_right[1], settings.MARGIN)
            ...
            if len(self.points_right) == 0 and bola_permitida_drch:
                ...
                right_y = random.uniform(
                    self.shoulder_left[1], settings.MARGIN)
                ...
                if not right_tramp:
                    ...
                    self.right_point = Sticker(
                        self.game.display, settings.BOMBA, right_x, right_y, 75, 75, True)
