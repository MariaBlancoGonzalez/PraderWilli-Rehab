...
class CalibrationScene(Scene):
    def __init__(self, options):
        ...
        # Images
        self.body = Sticker(self.screen, settings.BODY,
                            settings.WIDTH/2, settings.HEIGHT/2, 1000, 760)

        # Puntos de la cabeza
        self.verde_cabeza = Sticker(
            self.screen, settings.VERDE, 640, 80, 60, 60)
        self.rojo_cabeza = Sticker(self.screen, settings.ROJO, 640, 80, 60, 60)

	# Puntos de las manos
        self.verde_izq_mano = Sticker(
            self.screen, settings.VERDE, 870, 400, 60, 60)
        self.rojo_izq_mano = Sticker(
            self.screen, settings.ROJO, 870, 400, 60, 60)

        self.verde_drch_mano = Sticker(
            self.screen, settings.VERDE, 410, 400, 60, 60)
        self.rojo_drch_mano = Sticker(
            self.screen, settings.ROJO, 410, 400, 60, 60)

	# Puntos de los pies
        self.verde_drch_pie = Sticker(
            self.screen, settings.VERDE, 600, 700, 60, 60)
        self.rojo_drch_pie = Sticker(
            self.screen, settings.ROJO, 600, 700, 60, 60)

        self.verde_izq_pie = Sticker(
            self.screen, settings.VERDE, 680, 700, 60, 60)
        self.rojo_izq_pie = Sticker(
            self.screen, settings.ROJO, 680, 700, 60, 60)
    ...
    def calculate_final_point(self, results, right, left):
        final_right = (
            results.pose_landmarks.landmark[12].x+right, results.pose_landmarks.landmark[12].y)
        final_left = (
            results.pose_landmarks.landmark[11].x+right, results.pose_landmarks.landmark[11].y)
