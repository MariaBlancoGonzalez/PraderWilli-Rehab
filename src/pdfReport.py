...
from utils import distribute_data, calculate_media_parte, calculate_media_total, get_best_score
from plots import create_pdf_graphics
...
class MyDocTemplate(BaseDocTemplate):
    ...
    def createPageTemplate(self):
        ...
        template = PageTemplate(
            id='mainTemplate', frames=frame, onPage=self.addHeader)
        return template
    ...
    def addHeader(self, canvas, doc):
        ...
        canvas.drawCentredString(
            doc.pagesize[0] / 2.0, doc.pagesize[1] - 40, f'Datos sobre {self.user} sobre la actividad {self.exercise}. Día {datetime.now().strftime("%d/%m/%Y")}')
        canvas.restoreState()

    def parse_data(self, data):
        data_parsed = [['Fecha', 'Tiempo', 'Fallos izquierda', 'Aciertos izquierda',
                    'Fallos derecha', 'Aciertos derecha']]
        for tup in data:
            new_list = list(tup)
            new_list.pop(0)
            new_list.pop(0)
    ...
    def get_statistics(self):
        time, left_error, left_correct, right_error, right_correct = distribute_data(self.data)
        ...
        media_error_left, media_error_right = calculate_media_parte(left_error), calculate_media_parte(right_error)
        media_correct_left, media_correct_right = calculate_media_parte(left_correct), calculate_media_parte(right_correct)
        line = '---------------------------------------------------------------------------------------------------------------------------------------'
        stats_str = f'{line}<br/>Mejor marca: <b>{best_score}</b><br/> {line}<br/>Mejor día de realización: <b>{best_day}</b><br/>{line}<br/>' + \
        ...
        plot_left = create_pdf_graphics(
            left_error, left_correct, time, 'izquierda')
        plot_right = create_pdf_graphics(
            right_error, right_correct, time, 'derecha')
    ...
    def create_table(self, data):
        ...
        c_width = [0.9*inch, 0.7*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch]
    ...
    def create_doc(self, doc):
        ...
        init_par = Paragraph(
            f'En este informe se recogen los últimos días de la actividad {self.exercise} de {self.user}. A continuación, se presenta en forma de tabla los datos recogidos por dias.',  styles['Normal'])
        ...
        doc_organization = [tableHeader_paragraph, init_par, self.space, table,
                            self.space, plotsHeader_paragraph, self.space, img_left, self.space, img_right, self.space, dataHeader_paragraph, stats_paragraph]
